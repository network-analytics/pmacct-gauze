...
pub struct Node<Pfx: Prefix> {
    ...
    parent_len: Option<u8>,
}

#[derive(Debug, Ord, PartialOrd, Eq, PartialEq)]
pub struct ClosestMatch<Pfx> {
    node_parent: Option<TreeRef<Pfx>>,
    node: TreeRef<Pfx>,
    branch: Option<Branch>,
}

#[derive(Debug, Ord, PartialOrd, Eq, PartialEq)]
pub struct FoundMatch<Pfx> {
    node_parent: Option<TreeRef<Pfx>>,
    node: TreeRef<Pfx>,
}
...
impl<Pfx: Prefix> Debug for Node<Pfx> {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        ...
        debug.field("parent_len", &self.parent_len.as_ref());
...
pub fn common_node<Pfx: Prefix>(node: &TreeRef<Pfx>, route: &Pfx) -> TreeRef<Pfx> {
    ...
    TreeRef::new(Structural, common_route, None, None, node.borrow().parent_len.clone())
}
...
pub fn set_child_node_and_parent<Pfx: Prefix>(parent: TreeRef<Pfx>, child: TreeRef<Pfx>) {
    ...
    child.borrow_mut().parent_len = Some(parent.borrow().prefix.prefix_len());
}

pub fn insert_parent_above<Pfx: Prefix>(grandparent: Option<TreeRef<Pfx>>, parent: TreeRef<Pfx>, child: TreeRef<Pfx>) {
    set_child_node_and_parent(parent.clone(), child);
    if let Some(old_parent) = grandparent && old_parent != parent {
...
impl<Pfx: Prefix> PrefixTree<Pfx> {
    ...
    pub fn lookup(&self, prefix: &Pfx) -> LookupResult<ClosestMatch<Pfx>, FoundMatch<Pfx>> {
        ...
        while root.borrow().prefix.contains(prefix) && root.borrow().prefix.prefix_len() <= prefix.prefix_len() {
            ...
            if root.borrow().prefix.prefix_len() == prefix.prefix_len() {
                return LookupResult::Found(FoundMatch {
                    node: root,
                    node_parent: parent,
                });
            }
            ...
            root = {
                ...
                let new_root = match branch_value {
                    None => return LookupResult::ClosestMatch(ClosestMatch { node_parent: parent, node: root.clone(), branch: Some(branch) }),
    ...
    pub fn insert(&mut self, prefix: Pfx) {
        ...
        let (root_parent, root) = match lookup {
            ...
            LookupResult::Found(FoundMatch { node, .. }) => {
            ...
            LookupResult::ClosestMatch(ClosestMatch { node_parent, node, branch, .. }) => {
                match branch {
                    // The prefix we looked up is supposed to be on branch 'branch' of our 'node'
                    Some(branch) => {
                        let pfx_len = node.borrow().prefix.prefix_len();
                        node.borrow_mut().set_branch(
                            branch,
                            Some(TreeRef::new(Entry, prefix, None, None, Some(pfx_len))),
                        );
                        return;
                    }

                    // The prefix we looked up is supposed to be in place of 'node'
                    None => (node_parent, node),
                }
            }
        };
        ...
        let potential_top = if prefix.contains(&root.borrow().prefix) {
            let new_node = TreeRef::new(Entry, prefix, None, None, root.borrow().parent_len);

            // Since new_node contains node we make it the parent of node
            insert_parent_above(root_parent, new_node.clone(), root);
            ...
            let new_node = TreeRef::new(Entry, prefix, None, None, common_node.borrow().parent_len);

            // Put the children on the branch they belong on
            insert_parent_above(root_parent, common_node.clone(), root);
        ...
        // table top
        if potential_top.borrow().parent_len.is_none() {
            self.set_top(Some(potential_top))
        };
    }

    pub fn delete(&mut self, prefix: &Pfx) {
        let (node_parent, node) = match self.lookup(prefix) {
            // If we do not find the exact prefix we have nothing to remove
            LookupResult::Empty
            | LookupResult::ClosestMatch { .. } => return,
            LookupResult::Found(FoundMatch { node, .. }) if node.borrow().node_type == Structural => return,
            LookupResult::Found(FoundMatch { node, node_parent }) => (node_parent, node),
        };
        ...
        if let Some(child) = &child {
            child.borrow_mut().parent_len = parent.as_ref().map(|parent| parent.borrow().parent_len).flatten();
        }
